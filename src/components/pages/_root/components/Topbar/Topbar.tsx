        ...
        <nav className="flex md:hidden items-center justify-between p-6 bg-base-100 border-b border-base-300 w-full sticky top-0 z-50 transition-shadow hover:shadow-2xl shadow-md">
            ...
            <div className="flex items-center gap-6">
                <button
                    onClick={() => signOut()}
                    disabled={isPending}
                    className="btn btn-ghost btn-circle hover:bg-error/10 hover:text-error transition-all p-2 rounded-full active:scale-[0.8] hover:rotate-12 duration-300"
                >
                    {isPending ? (
                ...
                <Link to={`/profile/${user?.id}`} className="avatar active:scale-90 transition-transform duration-300 p-0.5 ring-2 ring-primary/20 rounded-full">
